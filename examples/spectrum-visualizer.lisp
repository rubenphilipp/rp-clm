...
(let* ((sndfile (path-from-same-dir "something.wav"))
       ...
       (fftsize 4096)
       (frame-rate 25)
       (duration (min (clm:sound-duration sndfile) 1.0))
  ...
  (with-stopwatch ()
    (let* ((anafil? (probe-file anafile))
           (ana? (not (when anafil?
                        (y-or-n-p "The analysis file ~a already exists. Use ~
                                   this file (y) or redo analysis (n)?"
                                  anafile)))))
      (when ana?
    ...
    (lparallel:pdotimes (i frames)
      (with-stopwatch ()
        (let* ((canvas (make-svg-toplevel :width width
               ...
               (num-points num-bins))
          ;;; make multiple paths
          (loop with heights = (loop repeat 60
                                     collect (+ 80 (random 100)))
